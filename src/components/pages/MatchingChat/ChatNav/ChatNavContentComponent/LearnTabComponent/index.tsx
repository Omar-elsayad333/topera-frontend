...
const LearnTabComponent = ({ archive }: { archive: boolean }) => {
  ...
  const getPageData = async () => {
    try {
      const endpoint = archive ? `/matching/archived/${MatchingEnum.LEARN}` : `/matching/${MatchingEnum.LEARN}`
      const res = await getHandler({ endpoint })
      setlearnMatchingData([
  ...
  return (
    <>
      {loading ? (
        ...
        <List>
          {learnMatchingData?.length > 0 &&
            learnMatchingData.map((section: any) => {
